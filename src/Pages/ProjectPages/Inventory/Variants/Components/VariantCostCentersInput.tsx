...
import VariantCostCenterModel from "../../../../../interfaces/ProjectInterfaces/Inventory/Variants/VariantCostCenterModel";
import { Add, Delete } from "@mui/icons-material";
import { v4 as uuid } from "uuid";

const VariantCostCentersInput: React.FC<{
  formType: FormTypes;
  variantCostCenters: VariantCostCenterModel[];
  handleUpdate: (costCenters: VariantCostCenterModel[]) => void;
  ...
  const createCostCenter = (): VariantCostCenterModel => ({
    id: uuid(),
    costCenterId: null,
    percent: 0,
  });
  ...
  const handleCostCenterChange = (
    ...
    const updated = variantCostCenters.map((item) => {
      if (item.id === id) {
  ...
  const handleDelete = (id: string | undefined) => {
    handleUpdate(variantCostCenters.filter((item) => item.id !== id));
  };

  const handleAdd = () => {
    handleUpdate([...variantCostCenters, createCostCenter()]);
  };
  // Render a single entry for cost center
  const renderCostCenter = (item: VariantCostCenterModel, index: number) => (
    <div
  ...
  return (
    <div className="card card-body">
      ...
      {variantCostCenters.map((item, index) => renderCostCenter(item, index))}
      {formType !== FormTypes.Details && (
        <div>
...
export default VariantCostCentersInput;
