        ...
        maxQuantity: yup
          ...
          .test(
            ...
            function (value) {
              ...
              return value != null && minQuantity != null && value > minQuantity;
            },
          ),
        enableNotification: yup.boolean().required(),
      }),
    ),
    expiryLevels: yup.array().of(
      yup.object().shape({
        level: yup.number().required(),
      ...
      function (levels) {
        ...
        for (let i = 0; i < sorted.length; i++) {
          if (i > 0 && sorted[i].level !== sorted[i - 1].level + 1) return false;
          for (let j = i + 1; j < sorted.length; j++) {
            const a = sorted[i];
            const b = sorted[j];
            if (a.daysFrom != null && a.daysTo != null && b.daysFrom != null && b.daysTo != null) {
              if (a.daysFrom <= b.daysTo && b.daysFrom <= a.daysTo) return false;
            }
          }
        }
        return true;
